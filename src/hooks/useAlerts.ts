...
export const useAlerts = () => {
  ...
  const requestLocation = useCallback(() => {
    // Reserva: se o GPS preciso falhar (ex: sem HTTPS/localhost — obrigatório para o
    // navegador conceder GPS — ou permissão negada), tentamos localização aproximada
    // por IP em vez de mostrar logo o ecrã de "geolocalização bloqueada". A mesma
    // lógica que já usamos na página principal.
    const tryApproximateFallback = async () => {
      try {
        const approx = await fetchApproximateLocation();
        if (approx) {
          logger.log(`[SOS Mais] GPS preciso indisponível — a usar localização aproximada por IP.`);
          setUserLocation({ lat: approx.lat, lng: approx.lon });
