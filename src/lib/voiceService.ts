...
export class VoiceService {
  ...
  private speakWithLocalBrowser(text: string) {
    ...
    utterance.onerror = (event) => {
      // A síntese de voz do WebView do Android é conhecida por falhar em
      // silêncio (sem vozes disponíveis, mesmo com o TTS do sistema instalado
      // e a funcionar noutras apps) — registamos o erro real em vez de
      // simplesmente desistir sem explicação, para sabermos se é isto.
      logger.warn('[VoiceService] Falha na síntese de voz:', event.error || event);
  ...
  public async speak(text: string) {
    ...
    if (!this.synth) {
      logger.warn('[VoiceService] speechSynthesis não está disponível neste WebView — o guia de voz não pode falar aqui.');
      return;
    }
    if (this.synth.getVoices().length === 0) {
      // Limitação conhecida do WebView do Android: por vezes não expõe nenhuma
      // voz via Web Speech API, mesmo que o telemóvel tenha TTS instalado e a
      // funcionar normalmente noutras apps. Isto não se corrige só com código —
      // precisaria de um plugin nativo de TTS (ex: @capacitor-community/text-to-speech).
      logger.warn('[VoiceService] Nenhuma voz disponível — provável limitação do WebView nativo, não um erro da app.');
    }
