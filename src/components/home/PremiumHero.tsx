  ...
  useEffect(() => {
    ...
    const fetchLocationByIP = async () => {
      try {
        ...
        // Busca temperatura real também no modo aproximado — antes ficava sempre fixa em 18°.
        let temp = 18;
        try {
          const weatherResponse = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.lon}&current_weather=true`, { signal: AbortSignal.timeout(3000) });
        ...
        setWeather(prev => {
          ...
          return {
            temp,
            status: 'IP Estimado',
            // Localização por IP não dá precisão ao nível da rua — evitamos fingir que
            // ainda estamos "a localizar"; isto só muda se o GPS real vier a responder.
            street: 'Zona aproximada',
      ...
      watchId = navigator.geolocation.watchPosition(async (pos) => {
        ...
        try {
          ...
          if (geoResponse.ok) {
            ...
            // Quando a precisão é baixa (ex: sem chip GPS, localização por Wi-Fi), nunca
            // mostramos um nome de rua específico — isso passa uma falsa sensação de
            // exatidão. Mostramos só a zona/cidade, que é o que a precisão real permite.
            let street: string;
            if (accuracyClass === 'APPROXIMATE') {
              street = area || data.name || "Zona aproximada";
            } else {
              street = mainStreet ? (addr.house_number ? `${mainStreet}, ${addr.house_number}` : mainStreet) : (area || data.name || "Localização Atual");
            }
