        ...
        if (profile.uid) {
          if ('geolocation' in navigator) {
            navigator.geolocation.getCurrentPosition(
              (position) => registerTokenWithBackend(profile.uid, token, position.coords.latitude, position.coords.longitude),
              () => registerTokenWithBackend(profile.uid, token),
 ...
 */
export async function triggerSOS(userId: string, latitude: number, longitude: number, address?: string) {
  try {
    const authHeader = await getAuthHeader();
